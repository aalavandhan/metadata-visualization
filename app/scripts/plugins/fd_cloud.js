...
Bubbles = function(width, height, topN, order) {
  ...
  chart = function(selection) {
    return selection.each(function(rawData) {
      ...
      if(order == 0){
        ...
        data = _.sortBy(data, function(d){ return order * d.count; })
      };
  ...
  update = function() {
    data.forEach(function(d, i) {
      return d.forceR = Math.max(minCollisionRadius, rScale(rValue(d)) /  2.35);
    });
  ...
  updateNodes = function() {
    ...
    node.exit().remove();
    return node.enter().append("a").attr("class", "bubble-node").call(force.drag).call(connectEvents).append("circle").attr("r", function(d) {
  ...
  updateLabels = function() {
    ...
    label.exit().remove();
    labelEnter = label.enter().append("a").attr("class", "bubble-label").call(force.drag).call(connectEvents);
    ...
    label.style("font-size", function(d) {
      return Math.max(4, rScale(rValue(d) / 50)) + "px";
    }).style("width", function(d) {
      return rScale(rValue(d) / 50) + "px";
    });
    label.append("span").text(function(d) {
      return textValue(d);
    }).each(function(d) {
      return d.dx = Math.max(rScale(rValue(d) / 50), this.getBoundingClientRect().width);
    }).remove();
